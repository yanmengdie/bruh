...
- [x] 将 app-facing functions、pipeline functions 和 cron 调度完整迁到自有服务器，摆脱 Supabase Functions 与 Supabase Cron。
  已完成：服务器已部署 `feed`、`generate-message`、`message-starters`、`generate-post-interactions`、`build-feed`、`build-news-events`、`ingest-top-news`、`ingest-x-posts` 八个本地 systemd 服务，并通过统一 functions gateway 暴露 `/functions/v1/*`。原先 Supabase `pg_cron/pg_net` 调度已经被 `/etc/cron.d/bruh-selfhost` 替代，当前定时任务在服务器本地直接调用函数，不再依赖 hosted Supabase 调度层。
...
- [x] 下线高风险小红书抓取链路，改用微博/X 等更稳定的数据源。
  已完成：因小红书账号风控风险过高，已从服务器 cron 移除小红书同步任务，删除服务器侧 XHS 登录态，并通过 `0021_retire_yingshi_liu_jingkang.sql` 下线仅依赖小红书的 `影石刘靖康` persona。当前进一步移除仓库内 `tools/xhs`、`scripts/run_xhs_sync.sh`、`scripts/push_xhs_auth_to_server.mjs` 和 `ingest-xhs-posts` 函数，避免后续误部署或误启用；同时清理服务器上的 `bruh-ingest-xhs-posts.service`、gateway 路由和 repo 副本残留。首页团队的小红书外链入口属于产品展示，不属于抓取链路，保持不变。
- [x] 清理已下线 persona 的数据库残留，保持自建库与当前产品名单一致。
  已完成：补充 `0023_retire_kobe_cleanup_residue.sql` 与 `0024_remove_zuckerberg_residue.sql`，并已在自建 PostgreSQL 上执行；当前 `kobe_bryant`、`zuckerberg`、`liu_jingkang`、`影石刘靖康` 在 `personas`、`persona_accounts`、`source_posts`、`feed_items`、`persona_news_scores`、`feed_comments`、`feed_likes` 中的残留均为 `0`。随后已同步相关后端代码到服务器并手动触发 `build-feed`，返回 `{"ok":true,"sourcePosts":240,"feedItems":240}`。
