...
function aggregateEvents(rows: NewsArticleRow[]) {
  ...
  for (const row of rows) {
    ...
    if (!existing) {
      clusters.set(eventKey, {
        ...
        interestTags: [...new Set(inferInterestTags(`${row.title} ${row.summary}`, row.category).concat(row.interest_tags ?? []))],
        representativeUrl: row.article_url,
        representativeSourceName: row.source_name,
        articleIds: [row.id],
        articleCount: 1,
        sourceCount: 1,
        importanceScore: row.importance_score + scoreNewsRecency(row.published_at),
    ...
    existing.importanceScore = Math.max(existing.importanceScore, row.importance_score + scoreNewsRecency(row.published_at)) + 0.35
    existing.interestTags = [...new Set(existing.interestTags.concat(row.interest_tags ?? []))]
  ...
  return [...clusters.values()]
    .sort((left, right) => right.importanceScore - left.importanceScore || Date.parse(right.publishedAt) - Date.parse(left.publishedAt))
}
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const hoursRaw = Number.parseInt(asString(body.lookbackHours) || "72", 10)
    const lookbackHours = Number.isNaN(hoursRaw) ? 72 : Math.min(Math.max(hoursRaw, 12), 168)
    const sinceIso = new Date(Date.now() - lookbackHours * 60 * 60 * 1000).toISOString()

    const supabase = createClient(projectUrl, serviceRoleKey)
    const { data: rows, error } = await supabase
      .from("news_articles")
      .select("id, title, summary, article_url, source_name, category, interest_tags, published_at, importance_score")
    ...
    if (topEvents.length > 0) {
      const eventRows = topEvents.map((event, index) => ({
        ...
        importance_score: Number((event.importanceScore + event.articleCount * 0.25).toFixed(3)),
      ...
      const joinRows = topEvents.flatMap((event) => event.articleIds.map((articleId) => ({
      ...
      const scoreRows = topEvents.flatMap((event) =>
        allPersonaIds().map((personaId) => {
          const scoring = scorePersonaForEvent(personaId, event.title, event.summary, event.interestTags)
          return {
            event_id: event.eventId,
            persona_id: personaId,
            score: Number((scoring.score + scoreNewsRecency(event.publishedAt)).toFixed(3)),
