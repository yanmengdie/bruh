...
private struct ContactsView: View {
    ...
    private func acceptInvitation(_ invitation: BruhInvitation) {
        ...
        if !wasAccepted && invitation.personaId == "trump" {
            ...
            DemoOrchestrator.shared.onTrumpAccepted(modelContext: modelContext)
        }

        if !wasAccepted && invitation.personaId == "musk" {
            DemoOrchestrator.shared.onElonAccepted(modelContext: modelContext)
        }
